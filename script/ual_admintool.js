        ...
        if($('#home-page').length>0) {			
			...
			$('input[type="checkbox"]').live("click", function() {
			    if($('#grid_copy_records').length>0) {
					// record ids for hiding
					if($(this).is(':checked')) {
					    //$(this).parent().children('.hide_sort').html('0');
						//$this).parent().parent('tr').children().css('background-color','#C00000');
					} else {
						//$(this).parent().children('.hide_sort').html('1');
						//$(this).parent().parent('tr').children().css('background-color','#fff');
					}
				}
			});
            ...
            $('#ok').live("click", function() {
                ...
                if(typeof(step_action)!='undefined'){
                    ...
                    $('div.container fieldset').html('<h1>Loading, please wait ...</h1>');
