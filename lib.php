    ...
    if ($result = $mysqli->query($workflow_data_details)) {
        while($row = $result->fetch_object()) {
            ...
            if($row->type=='text') {
                ...
                if($row->mandatory==1){
                    $workflow_form .= '<em>*</em><label for="'.$row->name.'">'.$row->label.'</label><input class="required" type="text" id="'.$row->name.'" name="'.$row->name.'">';
                } else {
            ...
            if($row->type=='dropdown') {
                ...
                // TODO: check if mandatory==1
                if($row->mandatory==1){
                    $workflow_form .= '<em>*</em><label for="'.$row->name.'">'.$row->label.'</label><select class="required" id="'.$row->name.'" name="'.$row->name.'">';
                } else {
