...
function get_filter_data($type=false, $data=false) {
    ...
    if($type==false) {
        ...
        // filter by programme 
        if($type=='P') {
            $courses_sql = "select distinct c.aos_code as id, c.full_description as name from courses c inner join enrolments e on e.studentid='$loggedin_username' inner join course_structure cs on aoscd_link=c.aos_code and cs.aos_code='$data' and c.courseid=e.courseid order by name";
        }
    }
